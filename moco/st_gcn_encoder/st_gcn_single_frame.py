...
def should_use_ghost(kernel_size, ghost_mode):
    if ghost_mode == "all":
        return True
    if ghost_mode == "kernel1":
        return kernel_size == 1 or kernel_size == (1, 1)
    if ghost_mode == "gt1":
        if isinstance(kernel_size, tuple):
            return any(k > 1 for k in kernel_size)
...
class Model(nn.Module):
    ...
    def __init__(self, opt):
        ...
        A = torch.tensor(self.graph.A, dtype=torch.float32, requires_grad=False)  # K, T*V, T*V
    ...
    def forward(self, x):
        ...
        #
        # # upsample
        # x_up = torch.zeros((N * M, fc_out, T, V), device=x.device)
        # for i in range(len(self.graph.part)):
        #     num_node = len(self.graph.part[i])
...
if __name__=='__main__':
    ...
    model = Model(opt=opt)
    src = torch.randn(16, 12, 21, 2).to(device)
    model.to(device)
