...
class NMFs_CSL(torch.utils.data.Dataset):
    def __init__(self,
                 data_root='/data/NMFs-CSL',
                 data_split='train',
                 hand_side='left',
                 interval=2,
                 threshold=0.5,
        ...
        '''
        NMFs_CSL dataset pre-processing for classification
        :param data_root: NMFs_CSL Dataset Root Path
        ...
        if self.data_split == 'train':
            video_list = np.genfromtxt(os.path.join(self.data_root, 'traintestlist', 'trainlist01.txt'), delimiter=' ',
                                       dtype=str)
            self.annotation_path = os.path.join(self.data_root, 'Joint_2D_mmpose')
    ...
    def __str__(self):
        info = "NMFs_CSL {} set. lenth {}".format(
    ...
    def extractvideoinfo(self, index, video_name):
        if self.use_cache:
            ...
            ori_img_size = self.pkl_data[video_name]['img_size']
        else:
            ...
            if self.data_split == 'train':
                ...
                label = int(video_index.split('/')[0])
            if label < 610:
                ori_img_size = np.array([[1000.0, 720.0]], dtype=np.float32)
            else:
                ori_img_size = np.array([[540.0, 540.0]], dtype=np.float32)
    ...
    def get_single_hand(self, index, total_frame_list):
        ...
        if label < 610:
            ...
            ori_img_size = np.array([[540.0, 540.0]], dtype=np.float32)
        video_data, _ = self.extractvideoinfo(index, video_name)
