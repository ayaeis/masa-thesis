...
def main():
    ...
    if args.distributed:
        ...
        ddp_device_ids = [args.gpu] if device.type == "cuda" else None
        model = nn.parallel.distributed.DistributedDataParallel(model, device_ids=ddp_device_ids, find_unused_parameters=True)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        loss_joint, top1_joint, loss_hand_2d, loss_body_2d = train(
            train_loader, model, criterion, optimizer, epoch, scaler, lambda_CL, device, args
        )
...
def train(train_loader, model, criterion, optimizer, epoch, scaler, lambda_CL, device, args):
    ...
    for i, (input_v1, input_v2) in enumerate(train_loader):
        ...
        for k, v in input_v1.items():
            for k_1, v_1 in v.items():
                input_v1[k][k_1] = v_1.float().to(device, non_blocking=(device.type == "cuda"))
        for k, v in input_v2.items():
            for k_1, v_1 in v.items():
                input_v2[k][k_1] = v_1.float().to(device, non_blocking=(device.type == "cuda"))
